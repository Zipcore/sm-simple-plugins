...
new g_aPlayer_Respawn[MAXPLAYERS + 1];

public Client_EnableRespawn(client)
{
	g_aPlayer_Respawn[client] = true;
...
public Client_EnableRespawnAll()
{
	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsValidClient(i))
		{
			Client_EnableRespawn(i);
		}
	}
}

public Client_DisableRespawn(client)
{
	g_aPlayer_Respawn[client] = false;
}

public Client_DisableRespawnAll()
{
	for (new i = 1; i <= MaxClients; i++)
	{
		if (IsValidClient(i))
		{
			Client_DisableRespawn(i);
		}
	}
}