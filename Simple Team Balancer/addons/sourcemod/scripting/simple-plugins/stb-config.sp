...
stock ProcessConfigFile()
{
	...
	BuildPath(Path_SM, sConfigFile, sizeof(sConfigFile), "configs/simple-teambalancer.cfg");
	if (!FileExists(sConfigFile)) 
	{
		/**
		Config file doesn't exists, stop the plugin
		*/
		LogError("[STB] Simple Team Balancer is not running! Could not find file %s", sConfigFile);
	...
	else
	{
		...
		*/
		...
		if (result != SMCError_Okay) 
		{
			...
			LogError("[STB] %s on line %d, col %d of %s", error, line, col, sConfigFile);
			LogError("[STB] Simple Team Balancer is not running! Failed to parse %s", sConfigFile);
...
public SMCResult:Config_NewSection(Handle:parser, const String:section[], bool:quotes) 
{
	...
	if (StrEqual(section, "global", false))
	{
	...
	if (g_eConfigSection == Section_Map)
	{
		switch (g_eConfigState)
		{
			case Reading_Integers:
			{
				...
				{
					LogError("Invalid key used in map section.");
					return SMCParse_Continue;
				}
			}
